#!/usr/bin/env python3
"""
Simple test script for PNG compression using pngcrush.
"""
...
def compress_png_with_pngcrush(source_path, dest_path, effort_level=2):
    """
    ...
    Args:
        ...
        effort_level: pngcrush effort level (0-4, default 2)
    ...
    try:
        ...
        # Build pngcrush command
        # -reduce: try to reduce the number of colors
        # -blacken: remove unused colors
        # -rem allb: remove all ancillary chunks
        # -l 9: use zlib compression level 9 (maximum compression)
        # -m effort_level: use specified effort level
        cmd = [
            "pngcrush",
            "-reduce",
            "-blacken",
        ...
        if result.returncode != 0:
            return {
                "success": False,
                "error": f"pngcrush failed: {result.stderr}",
        ...
        return {
            "success": False,
            "error": "pngcrush timed out after 5 minutes",
...
def main():
    parser = argparse.ArgumentParser(description="Compress PNG files using pngcrush")
    ...
    parser.add_argument("--effort-level", type=int, default=2, choices=range(5), 
                       help="pngcrush effort level 0-4 (default: 2)")
    ...
    print("PNG Compression Test (pngcrush)")
    ...
    for i, png_file in enumerate(png_files, 1):
        ...
        result = compress_png_with_pngcrush(png_file, compressed_file, args.effort_level)
