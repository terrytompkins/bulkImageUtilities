#!/usr/bin/env python3
"""
Simple zstd archiving test script.
...
def create_zstd_archive(batch_dir, output_path, compression_level):
    """Create a zstd archive from a batch directory."""
    try:
        ...
        # Archive the PNG files directly from the batch directory
        file_names = " ".join([f.name for f in png_files])
        cmd = f"tar -cf - {file_names} | {zstd_cmd} > {output_path.absolute()}"
...
def main():
    parser = argparse.ArgumentParser(description="Test zstd archiving on PNG files")
    ...
    parser.add_argument("--compression-level", type=int, default=9, choices=range(1, 23), help="zstd compression level 1-22 (default: 9)")
    ...
    for i, batch_dir in enumerate(batch_dirs, 1):
        archive_path = output_dir / f"part{i:02d}.tar.zst"
        
        print(f"  Creating archive {i}/{len(batch_dirs)}: {archive_path.name}")
        
        result = create_zstd_archive(batch_dir, archive_path, args.compression_level)
