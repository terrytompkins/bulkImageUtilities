#!/usr/bin/env python3
"""
Simple test script for PNG compression using zopflipng.
"""
...
def compress_png_with_zopflipng(source_path, dest_path, effort_level=15):
    """
    Compress a single PNG file using zopflipng.
    
    Args:
        source_path: Path to source PNG file
        dest_path: Path for output compressed PNG file
        effort_level: zopflipng iterations (1-15, default 15)
    ...
    try:
        ...
        # Build zopflipng command using simple two-argument format
        # -y: do not ask about overwriting files
        # -m: compress more (uses more iterations)
        cmd = [
            "zopflipng",
            "-y",
            "-m",
        ...
        if not dest_path.exists():
            return {
                "success": False,
                "error": f"zopflipng failed: {result.stderr}",
        ...
        return {
            "success": False,
            "error": "zopflipng timed out after 5 minutes",
...
def main():
    parser = argparse.ArgumentParser(description="Compress PNG files using zopflipng")
    ...
    parser.add_argument("--effort-level", type=int, default=15, choices=range(1, 16), 
                       help="zopflipng iterations 1-15 (default: 15)")
    ...
    print("PNG Compression Test (zopflipng)")
    ...
    for i, png_file in enumerate(png_files, 1):
        ...
        result = compress_png_with_zopflipng(png_file, compressed_file, args.effort_level)
